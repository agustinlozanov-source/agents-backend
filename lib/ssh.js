...
export async function connectVPS() {
  ...
  connectionPromise = (async () => {
    try {
      ...
      // Reset flag if connection closes unexpectedly
      ssh.connection?.on('close', () => {
        console.log('⚠️ SSH connection closed, will reconnect on next call');
        isConnected = false;
...
export async function executeCommand(command) {
  try {
    ...
    // Wrap in bash -c to support &&, nohup, & etc.
    const result = await connection.execCommand(`bash -c ${JSON.stringify(command)}`, {
      cwd: '/root/agente_ia'
    });

    console.log('SSH result:', { code: result.code, stdout: result.stdout?.slice(0, 200), stderr: result.stderr?.slice(0, 200) });
    ...
    // Reset connection state so next call reconnects
    isConnected = false;
    connectionPromise = null;
    ssh.dispose().catch(() => {});
